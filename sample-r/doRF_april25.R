...
##########################################################################################
### April 25 - run randomForest on 50k files 
##########################################################################################

load("data/raw.RData")
# Clean
raw.data$cleanedText = tolower(gsub("[[:punct:]]", "", raw.data$SentimentText))
finalTest.df = raw.data[raw.data$Sentiment == -1, ]

load("data/bow-april24/BOW_allWordFreq_all_april24.RData")
...
for (i in 1:n) {
  print(list("Current in loop: ", i))
  finalTest.df = getPattern(data = finalTest.df, sub.index = 1:nrow(finalTest.df), char.col = 5,
                                 pattern = importantWords[i], pat.name = importantWords[i])
}

### Clean up
finalTest.clean = data.frame(Sentiment = finalTest.df$Sentiment,
                                finalTest.df[ , seq(from = 7, to = ncol(finalTest.df), by = 2)])
BOW_finalTest_clean = finalTest.clean

# Add data to .rda files
...
save(april25.myPredict, file = "submission/April25_submission_100tree_model.rda")
submission = data.frame(id = 1:length(april25.myPredict), y = april25.myPredict)
write.csv(submission, file = "submission/april25_COP.csv", row.names = FALSE)
#
#
#

##########################################################################################
### April 25 - old randomForest script below...
##########################################################################################
